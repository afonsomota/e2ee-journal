...
│   ├── main.dart
│   ├── models/       Data classes
│   ├── services/     Auth, crypto, journal + FHE emotion orchestration
│   ├── fhe/          Native FHE client (Dart FFI → Rust/TFHE-rs)
│   └── screens/      UI
└── rust/             libfhe_client — Rust crate (keygen, encrypt, decrypt via TFHE-rs)

journal_backend/      FastAPI server (Python)
├── main.py
├── models/           Database schema
└── routers/          API endpoints (including /fhe/key and /fhe/predict)
```
...
```bash
source emotion_ml/.venv/bin/activate
...
model artifacts. Requires `concrete-ml` (installed in `emotion_ml/.venv`).
